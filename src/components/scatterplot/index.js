...
const Scatterplot = (props) => {
  const MAX_CHART_SIZE = { x_axis: props.axis_x_max, y_axis: props.axis_y_max };
  const [testdata, setTestdata] = useState([
    {
      name: "X",
      type: "line",
      symbolSize: 0,
      data: props.axis_y_divider? ([
        [0, MAX_CHART_SIZE.y_axis / 2],
        [MAX_CHART_SIZE.x_axis, MAX_CHART_SIZE.y_axis / 2],
      ]):([]),
      color: "#808080",
      silent: true,
      label: { show: false },
    ...
    },
    {
      ...
      type: "line",
      symbolSize: 0,
      data: props.axis_x_divider?([
        [MAX_CHART_SIZE.x_axis / 2, 0],
        [MAX_CHART_SIZE.x_axis / 2, MAX_CHART_SIZE.y_axis],
      ]):([]),
      silent: true,
      label: { show: false },
      color: "#808080",
  ...
  useEffect(() => {
    const fetchData = async () => {
      const scatterData = props.dataset || scatterdataset.scatterdata;
  ...
  return (
    <div>
      <ReactECharts option={option} style={{ height: props.height || 500 }} />
    </div>
  );
};
